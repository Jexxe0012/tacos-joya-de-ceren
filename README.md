...
2. La aplicación valida los datos y guarda la sesión en el navegador. Actualmente, ambos perfiles ingresan al módulo de Turnos, que presenta una vista distinta según el rol.
3. Desde el menú lateral se accede a los módulos. Dashboard y Reportes comprueban que la sesión corresponda a un administrador; Recetas y Turnos requieren una sesión activa.
...
Las vistas y acciones se adaptan al perfil. La navegación compartida muestra los enlaces de los módulos; las comprobaciones de acceso se realizan en las páginas protegidas.

## Módulos

### Autenticación

Incluye inicio de sesión, registro, validación de campos y diferenciación entre administrador y empleado. La sesión se guarda en `localStorage` y se consulta mediante `js/guard.js`. Si se intenta acceder a una página protegida sin sesión, la aplicación redirige al login; si un empleado intenta entrar a Dashboard o Reportes, vuelve a Turnos.
...
Sus páginas independientes están pendientes de implementación. Por ahora, el registro de mermas está disponible en la vista del empleado en Turnos y el Dashboard muestra un ejemplo de abastecimiento previsto.
...
3. Abre `pages/login.html` desde ese servidor. Si utiliza el puerto 5500, la dirección será `http://127.0.0.1:5500/pages/login.html`.
